...
from .neighgen import GeneticProbaGenerator, RandomGeneticProbaGenerator, GeneticProbaUncertaintyGenerator
...
class LOREM(object):
    ...
    def __init__(self, K, bb_predict, predict_proba, feature_names, class_name, class_values, numeric_columns, features_map,
                 ...
                 extreme_fidelity = False, unc_thr = 0.5, uncertainty_metric = 'max', constraints = None, **kwargs):
    ...
    def explain_instance_stable_neigh(self, x, Z_list, use_weights=True, metric=neuclidean, runs=3, exemplar_num=5,
                                n_jobs=-1, prune_tree=False,  extract_counterfactuals_by= 'min_distance', kwargs=None):
        ...
        if self.binary == 'binary_from_nari' or self.binary == 'binary_from_dts' or self.binary == 'binary_from_bb':
            Xc_final, crules, deltas = get_counterfactual_rules(x, Yc[0], superT, Z, Yc, self.feature_names,
                                                      self.class_name, self.class_values, self.numeric_columns,
                                                      self.features_map, self.features_map_inv, encdec=self.encdec,
                                                      filter_crules=self.filter_crules, bb_predict_proba = self.bb_predict_proba, 
                                                      unc_thr = self.unc_thr, uncertainty_metric = self.uncertainty_metric,
                                                      constraints=self.constraints, unadmittible_features=self.unadmittible_features,
                                                      extract_counterfactuals_by=extract_counterfactuals_by, metric = metric)
        else:
            Xc_final, crules, deltas = get_counterfactual_rules_supert(x, Yc[0], superT, Z, Yc, self.feature_names,
    ...
    def explain_instance_stable(self, x, samples=100, use_weights=True, metric=neuclidean, runs=3, exemplar_num=5,
        ...
        if self.binary == 'binary_from_nari' or self.binary == 'binary_from_dts' or self.binary == 'binary_from_bb':
            ...
            Xc_final, crules, deltas = get_counterfactual_rules(x, Yc[0], superT, Z, Yc, self.feature_names,
                                                          ...
                                                          bb_predict_proba = self.bb_predict_proba, unc_thr = self.unc_thr, uncertainty_metric = self.uncertainty_metric,
    ...
    def __init_neighbor_fn(self, ocr, categorical_use_prob, continuous_fun_estimation, size, kwargs):
        ...
        if self.neigh_type in ['random', 'genetic', 'rndgen', 'geneticp', 'rndgenp', 'counter', 'geneticpu']:
            if self.verbose:
        ...
        if self.neigh_type in ['genetic', 'rndgen', 'geneticp', 'rndgenp']:
        ...
        elif self.neigh_type == 'geneticpu':
            alpha1 = kwargs.get('alpha1', 0.33)
            alpha2 = kwargs.get('alpha2', 0.33)
            alpha3 = kwargs.get('alpha3', 0.33)
            uncertainty_metric = kwargs.get('uncertainty_metric', 'delta')
            metric = kwargs.get('metric', neuclidean)
            ngen = kwargs.get('ngen', 10)
            mutpb = kwargs.get('mutpb', 0.5)
            cxpb = kwargs.get('cxpb', 0.7)
            ...
            neighgen = GeneticProbaUncertaintyGenerator(self.bb_predict, self.feature_values, self.features_map, nbr_features,
                                                 nbr_real_features, numeric_columns_index, ocr=ocr, alpha1=alpha1,
                                                 alpha2=alpha2, alpha3=alpha3, 
                                                 uncertainty_metric=uncertainty_metric, metric=metric, ngen=ngen,
                                                 mutpb=mutpb, cxpb=cxpb, tournsize=tournsize,
                                                 halloffame_ratio=halloffame_ratio,
                                                 bb_predict_proba=self.bb_predict_proba,
                                                 random_seed=random_seed, encdec=self.encdec,
