...
def update_crules_by_min_sc(qlen, clen, xc, crule, delta, Xc_final, crule_list, delta_list):
    if qlen < clen:
        clen = qlen
        Xc_final = [xc]
        crule_list = [crule]
    ...
    return qlen, clen, xc, crule, delta, Xc_final, crule_list, delta_list
    
def update_crules_by_min_feature_distance(cfdist, x, xc, crule, delta, Xc_final, crule_list, delta_list, metric, bb_predict_proba):    
    ...
    if qdist < cfdist:
        ...
        crule_list = [crule]
        delta_list = [delta]
        print(bb_predict_proba(xc.reshape(1, -1)))
    elif qdist == cfdist:
        if delta not in delta_list:
            Xc_final.append(xc)
            crule_list.append(crule)
            delta_list.append(delta)
            print(bb_predict_proba(xc.reshape(1, -1)))
    return cfdist, xc, crule, delta, Xc_final, crule_list, delta_list
...
def get_counterfactual_rules(x, y, dt, Z, Y, feature_names, class_name, class_values, numeric_columns, features_map,
                             ...
                             unc_thr = 0.5, uncertainty_metric = 'max', constraints=None, unadmittible_features=None, 
                             extract_counterfactuals_by='min_sc', metric = neuclidean):
    clen, cfdist = np.inf, np.inf
    crule_list = list()
    delta_list = list()
    ...
    for z in Z1:
        ...
        if filter_crules is not None:
            ...
            if uncertainty_metric=='max':
                if np.max(bb_outcomec_probs) < unc_thr:
                    continue
            elif uncertainty_metric == 'delta':
            ...
            if bb_outcomec == dt_outcomec:
                if extract_counterfactuals_by == 'min_sc':
                    qlen, clen, xc, crule, delta, Xc_final, crule_list, delta_list = update_crules_by_min_sc(qlen, clen, xc, crule, delta, Xc_final, crule_list, delta_list)
                elif extract_counterfactuals_by == 'min_distance':
                    cfdist, xc, crule, delta, Xc_final, crule_list, delta_list = update_crules_by_min_feature_distance(cfdist, x, xc, crule, delta, Xc_final, crule_list, delta_list, metric, bb_predict_proba)
                else:
            ...
            if extract_counterfactuals_by == 'min_sc':
                ...
                print('Type of counterfactual not yet implemented')
    return Xc_final, crule_list, delta_list
        ...
        if filter_crules is not None:
            ...
            if qlen < clen:
                ...
                if delta not in delta_list:
                    ...
                    crule_list.append(crule)
                    delta_list.append(delta)
    return Z1_final, crule_list, delta_list
